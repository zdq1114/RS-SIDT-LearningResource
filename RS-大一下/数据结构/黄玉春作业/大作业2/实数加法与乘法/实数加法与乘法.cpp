...
void nLen_of_File(int &n1, int &n2, const char* path) {
	FILE *fp = nullptr;
	fopen_s(&fp, path, "rt");
	char* p = nullptr;
...
void CreatPolyn(polyn &P, int n1, int n2, const char *path) {
...
void CarryBit(polyn &p) {//进位
	...
	if (temp2 != 0) {
		...
		p.tail = p.tail->next;
		p.tail->data.coef = temp2;
		p.tail->data.expn = expn + 1;
		p.tail->next = NULL;
	}
}
...
void main() {
	...
	int alen1, alen2, blen1, blen2;
	nLen_of_File(alen1, alen2, path1); nLen_of_File(blen1, blen2, path2);
	polyn Pa, Pb;
	CreatPolyn(Pa, alen1, alen2, path1); CreatPolyn(Pb, blen1, blen2, path2);
	AddPolyn(Pa, Pb);
	...
	while (qa) {
		...
		if (qa->data.expn == -1)fprintf_s(fp, ".");
		qa = qa->next;
	}
